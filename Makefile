...
BUILDDIR	:= build/$(NAME)-$(VARIANT)
ELF		:= build/$(NAME)-$(VARIANT).elf
DUMP		:= build/$(NAME)-$(VARIANT).dump
MAP		:= build/$(NAME)-$(VARIANT).map
ROM		:= $(NAME)-$(VARIANT).nds
...
ARCH		:= -marm -mcpu=arm946e-s+nofp -g
...
LDFLAGS		+= $(ARCH) $(LIBDIRSFLAGS) -Wl,-Map,$(MAP) $(DEFINES) \
		   -Wl,--start-group $(LIBS) -Wl,--end-group -specs=$(SPECS) -Wl,--print-memory-usage
...
$(NAME)-$(VARIANT).sym: $(ELF)
	$(V)$(PREFIX)nm --numeric-sort $(ELF) | grep " [TtBbDd] " | awk '{print toupper($$1) " " $$3}' > $(NAME)-$(VARIANT).sym

$(ROM): $(ELF) $(NAME)-$(VARIANT).sym
	@echo "  NDSTOOL $@"
